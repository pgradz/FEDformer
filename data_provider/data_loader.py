...
class Dataset_crypto_hour(Dataset):
    ...
    def __read_data__(self):
        ...
        border1s = [0, index_train_end - self.seq_len+2, index_val_end - self.seq_len+2]
        border2s = [index_train_end+1, index_val_end+1, index_test_end+1]
        border1 = border1s[self.set_type]
        border2 = border2s[self.set_type]
...
class Dataset_crypto_triple_barrier(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 features='M', data_path='BTCUSDT_60min.csv',
                 target='close', scale=True, timeenc=0, freq='h',
                 train_end = 0, val_end = 0, test_end = 0):
    ...
    def __read_data__(self):
        ...
        df_raw.rename(columns={'datetime':'date'}, inplace=True)
        #time related features are added later
        columns_to_drop = [col for col in ['hour_sin','hour_cos','weekday_sin','weekday_cos',
                             'prc_change','barrier_touched','barrier_touched_date',
                             'top_barrier','bottom_barrier','time_step'] if col in df_raw.columns]
        df_raw.drop(columns=columns_to_drop, inplace=True)
    ...
    def __getitem__(self, index):
        ...
        seq_y = self.data_y[r_begin:r_end] # 48:96
        seq_x_mark = self.data_stamp[s_begin:s_end] # 0:95
        seq_y_mark = self.data_stamp[r_begin:r_end] # 48:119
        cls_y = self.data_y_cls[s_end-1] # 95 - target in row change between row 95 and 96. line 439 is commented

        return seq_x, seq_y, seq_x_mark, seq_y_mark, cls_y, s_end -1
        # return seq_x, seq_y, seq_x_mark, seq_y_mark, index # for debug to check that indexes in Subset are correct
...
class Dataset_crypto_hour_seq2seq(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 features='M', data_path='BTCUSDT_60min.csv',
                 target='close', scale=True, timeenc=0, freq='h'):
        # size [seq_len, label_len, pred_len]
    ...
    def __read_data__(self):
        ...
        if self.features == 'M' or self.features == 'MS':
            ...
            df_data = df_raw[[self.target]]
        # set index of target column so it can be retrieved in __getitem__    
        self.target_col_index = cols_data.index(self.target)
    ...
    def __getitem__(self, index):
        ...
        seq_x = self.data_x[s_begin:s_end]
        seq_y = self.data_y[s_end:r_end, self.target_col_index]
        seq_x_mark = self.data_stamp[s_begin:s_end]
